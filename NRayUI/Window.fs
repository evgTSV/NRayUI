...
type WindowParameters = {
    ...
    member this.Update() : unit =
        ...
        let struct (unscaledRenderTargetWidth, unscaledRenderTargetHeight) =
            WindowParameters.DefaultWindowSizeWithoutScale

        let renderTargetSize =
            struct (int (float32 unscaledRenderTargetWidth * scaleFactor.X),
    ...
    member this.Scale(units: float32) : float32 = units * this.ScaleFactor
