...
let scanSidesInput (input: string) =
    ...
    | _ -> failwithf $"Invalid input: %s{input}. Expected format: 'top;right;bottom;left'"
...
let scanCornersInput (input: string) =
    ...
    | _ ->
        failwithf
            $"Invalid input: %s{input}. Expected format: 'topLeft;topRight;bottomRight;bottomLeft"
